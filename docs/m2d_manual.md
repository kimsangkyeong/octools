...
색상 코드 정의(`C_RED`, `C_GREEN`, `C_YELLOW`, `C_CYAN`, `C_BOLD` 등)는 유지하되, 일부 터미널에서 글씨가 안 보이는 문제를 피하기 위해 **화면 출력은 모두 `C_BOLD`(굵게)로 통일** 되어 있습니다.

- 대상: `print_error` / `print_warn` / `print_info` / `print_ok` 및 Information 블록(헤더/구분선/실행 명령어 라인).
...
| v1.2 | 2026-09-07 | 화면 출력을 C_BOLD 로 통일(색상 정의는 유지, 가독성 문제 회피. 필요 시 수동으로 색상 변경) | k.s.k & kiro |
